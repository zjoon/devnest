...
function loadProduct() {
    ...
    if (!id || !products[id]) {
        document.getElementById('detailsContent').innerHTML = '<p style="color:white; text-align:center;">Nothing there go to <a href="index.html" style="color: #ff6b6b;">Home</a></p>';
        return;
    }
    currentId = id;
...
function showCartToast(message) {
    ...
    toast.innerHTML = `
        <span class="toast-icon">🛒</span>
        <span class="toast-text">${message}</span>
        <a href="cart.html" style="background: white; color: #b30000; border: none; border-radius: 20px; padding: 6px 14px; font-size: 12px; font-weight: bold; cursor: pointer; text-decoration: none; white-space: nowrap; flex-shrink: 0;">Go to Cart →</a>
        <button class="toast-close" onclick="this.parentElement.remove()">✕</button>
    `;
    container.appendChild(toast);
